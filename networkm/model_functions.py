...
__all__ = ['COPY', 'NOT', 'AND', 'OR', 'XOR2', 'XOR', 'sigmoid', 'graph_iter', 'dde_derivative',
           'dde_derivative_risefall', 'dde_integrator', 'parse_initial_conditions', 'plot_graph', 'graph_model', 'g',
           'x', 'g', 'x']
...
def dde_integrator(dt : float,
    ...
    #max_delay=int(np.max([np.max(i[-1]) for i in iterator]))
...
# Cell
print('Warming up integrator; subsequent calls will be accelerated.')
g = ring(N=3,right=True,left=True,loop=True)
x=graph_model(g,plot=False)
g=None
x=None